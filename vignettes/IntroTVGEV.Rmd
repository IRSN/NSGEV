...
30 years 2021 to 2050. This level is estimated here to be
...
$b_0^\star + n^\star$, such as the lifetime or *design life* 
[@RootzenKatz_DesignLife] of an
equipment, and one want to assess a risk related to the possible
exceedance over some high level during this period. Since the blocks
...
On the previous plot, confidence intervals based on the delta method
are shown. It is now possible to use profile likelihood intervals, at 
the price of an increased computation time.
...
Needless to say, for a small probability of exceedance, the upper
bound of the profile likelihood $95\%$ confidence interval is much
larger than that of the delta interval.
...
dependence between the blocks should be weak
[@CoxSnell_GeneralResiduals]. Several definitions of the residuals are
possible, in relation with a target distribution. A simple choice
is $e_b := F_{\text{GEV}}(y_b;\, \widehat{\boldsymbol{\theta}}_b)$
which should lead nearly to a uniform distribution on the interval
$(0,\,1)$. Other possible choices are $e_b := -\log[1 -
F_{\text{GEV}}(y_b;\, \widehat{\boldsymbol{\theta}}_b)]\}$ for a
standard exponential target as used by @PanagouliaEtAl_NonStationaryGEV
and by @KatzEtAl_ExtremesHydrology, or $e_b := \log\{-\log[1 -
F_{\text{GEV}}(y_b;\, \widehat{\boldsymbol{\theta}}_b)]\}$ for
a standard Gumbel target.  Note that the `resid` method of the package
uses an *increasing* transformation so that large/small residuals
correspond to large/small observations.  When the GEV marginal
distributions have a nearly exponential tail $\xi \approx 0$ using the
standard Gumbel target for the generalized residuals can be preferred
because this will use a milder transformation of the data. This is now
the default choice.
