...
to study *block maxima*, usually annual maxima. This kind of models
could be used with CRAN packages such as **extRemes**, yet **NSGEV**
makes it easier to cope with bases of functions. It also brings some
new things such as the profile likelihood confidence intervals on the
return levels or on the quantile of the maximum on an arbitrary
period.
...
We can fit a model with a linear time trend. A possibility is 
to use the `polynomX` function that creates a basis of polynomial
functions for a given degree. This makes a variable `t1` available
for the fit
...
does not require a data preparation step. By default the "new" dates
at which the return levels are computed are those in the data frame
used to fit the model


```{r, DijonPred, message=FALSE, warning=FALSE}
pred <- predict(fit1)
autoplot(pred)
...
using the `confintMethod` argument (possibly abbreviated as `confint`
of `conf`). Note that the `autoplot` method is to be used rather than
the `plot` method, because the **ggplot2* package is used.
...
period*. The distribution of $M$ is then no longer a GEV but the
its quantiles can be computed with confidence intervals
