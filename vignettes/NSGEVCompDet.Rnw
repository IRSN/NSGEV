...
future blocks $b^\star$. This period desgin life
period~\cite{RootzenKatz_DesignLife}.  Due to the independence of the
block maxima, the random maximum on the period, namely
\begin{equation}
  \label{eq:DefM}
...
depends linearly on $\bs{\psi}$ and the on the vector $\m{x}_b$ of
...
the height of the dike corresponding to $1\%$ risk of being exceeded
during the period.
...
to lie. This actually required to use the R function
...
\begin{equation*}
  ...
  \right]
\end{equation*}
where each ot the three blocks at r.h.s. is $B^\star \times 1$ and
...
\item Let $\m{G}$ be the $B^\star \times 3$ Jacobian matrix of the
  vector-valued function with value
  $[\log F_{\texttt{GEV}, b^\star}]_{b^\star}$, the differentiation
  being w.r.t. the three GEV parameters $\bs{\theta}^{[i]}$.
  \begin{equation*}
