...
\eqn{M^\star} does not follow a GEV distribution.
...
of the period is increased the maximum becomes stochastically larger: all
quantiles are increased. 

It is worth noting that as far as a small probability of exceedance
$\bar{p} := 1 -p$ is concerned, the expected number of the random
number $N$ of exceedances over the corresponding quantile is
$\mathbb{E}[N] = \bar{p}$. This approximation is acceptable when
$\bar{p} < 0.05$.
