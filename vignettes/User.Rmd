---
title: "Time-Varying Models in NSGEV DRAFT version a"
author: "Yves Deville"
date: "12/04/2019"
output:
  pdf_document:
...
library(ismev)
```
$\color{red}\text{XXX TODO. Citations: packages, more useful ref. Complete XXX}$.  
...
Value (GEV) distribution, usually block maxima. This class of models
is named `"TVGEV"`.  These models are specific Non-Stationary GEV
...
**ismev**, **evd**, **extRemes** and more. However with these
packages, the specificities of the Time-Varying context are not taken
into account, and it is quite difficult for instance to compute
...
existence of coefficient $\mu_0$ in $\mu(t) = \mu_0 + \mu_1 t$. We
...
Note however that we still require that the response *and the date* are
as column of a data frame.	
...
The $p$-value is `r ifelse(anova(fitNConst, fitNLin)[["Pr(>W)"]][2] > 0.05, "> 0.05", "< 0.005")`,
so if we assume that there could be a global linear trend on the
location, we would reject the null hypothesis that the slope $\mu_1$
differs from zero at the $5\%$ level.
...
a column of ones as commonly used in linear regression, and the column
`t1` is a linear trend in years with its origin set 49 years after
1921-01-01, so near 1970-01-01. Note that a `TVGEV` model does not
need to use all the basis functions of the design: we did not use the
constant `Cst`, because a constant basis function is generated by
...
- The date column is *passed to a design function* given in the `design` formal
  argument. The value of `design` is a call to the chosen function,
  using the chosen column of `data` as the `date` of the design
  function.
...
*nested*: by seting $\mu_2 = 0$ in the second model, we retrieve the
...
$\color{red}\text{XXX Explain and illustrate the need to set the time
origin}$.  
...
$\color{red}\text{XXX Explain and illustrate the Time-Varying return
level of Parey et al.}$.
...
plot(simulate(fitNBreak, nsim = 10, newdate = nd))
```
...
Several methods can be used to assess the marginal distribution of $Y_b$
for a specific time $t_b$.   

$\color{red}\text{XXX Illustrate the use of the mean, moment and density methods}$.
...
simulated or "resampled" corresponding to $r=1$, $\dots$, $R$. These
...
mu <- apply(myBoot$estimate, MARGIN = 1,
            ...
            model = fitNBreak)
autoplot(bts(mu)) + ggtitle("bootstrapped values for mu")
```
...
The **lubridate** package has functions that can help to cope with
date.
