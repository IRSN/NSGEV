...
##' 
##' @param model A \code{NSGEV} object.
##'
##' @param data A data frame containing covariates for the model.  By
...
##'
##' @param sampleData Logical. If \code{TRUE} the rows of \code{data}
...
##' 
##' @param plot Logical. If \code{type} is \code{"expect"} , the a plot
...
RL <- function(model,
    ...
    if (!sampleData) {
        ...
        ind <- sample(x = 1L:nrow(data), size = B, replace = TRUE)
        t_ind <- table(ind)
    ...
    if (type == "average") {
        ...
        if (is.null(rhoMax)) {
            ...
            rhoMax <- 3 * weighted.mean(q, w = w_ind)
        }
        
        res <- try(uniroot(f = g, interval = c(1.1, rhoMax)))
